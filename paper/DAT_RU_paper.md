...
**Running head:** DAT-RU: Practice Resistance and Lexical Predictors

---

## Abstract

The Divergent Association Task (DAT; Olson et al., 2021) measures verbal creativity by asking participants to name semantically unrelated words. We present DAT-RU, a Russian adaptation deployed as a web application, yielding 21,159 submissions from approximately 1,860 unique users over 50 days. **Study 1** establishes internal consistency (Cronbach's alpha = .90, split-half r_SB = .70) but low test-retest reliability (r = .23), suggesting divergent association is state-like rather than trait-like. Cross-linguistic calibration shows the 11-point mean score difference between Russian and English samples is largely genuine, not an embedding-model artifact. **Study 2** models repeated attempts as independent draws from a stable personal distribution: personal bests grow at or below the rate predicted by order statistics, with no evidence of learning or strategic improvement. Marginal returns collapse after 5-10 attempts (125-fold efficiency drop). **Study 3** identifies category diversity as the strongest predictor of DAT scores (r = .47), followed by minimum pairwise distance (the "weakest link"; d = 3.89 between top and bottom performers). A UMAP semantic map reveals that high scorers explore 44% more semantic territory, while most participants cluster around concrete "attractor" words. The theoretical ceiling (110.5) is approached by the best human score (104.8, 95%) but not by the average participant. Together, these findings characterize the DAT as a reliable single-session measure of semantic exploration breadth that resists strategic gaming but is sensitive to momentary cognitive state.

**Keywords:** divergent thinking, creativity, semantic distance, Divergent Association Task, word embeddings, practice effects, Russian language, psychometrics
...
### 2.2 Embedding Model and Vocabulary

Word embeddings were obtained from **navec** (Natasha Project; https://github.com/natasha/navec), specifically the `navec_hudlit_v1_12B_500K_300d_100q` model trained on 12 billion tokens of Russian literary texts. The model provides 300-dimensional vectors for approximately 500,000 word forms. We filtered the vocabulary to retain common nouns (excluding proper nouns, surnames, patronymics, toponyms, and abbreviations) using morphological analysis via pymorphy3 (Korobov, 2015). Word forms in any grammatical case or number were mapped to their nominative singular lemma. The character "yo" (e) was normalized to "ye" (e). The resulting vocabulary comprises 68,841 unique noun lemmas with associated word-form mappings (approximately 400,000 inflected forms).
...
$$\text{raw} = \frac{100}{21} \sum_{i=1}^{7}\sum_{j=i+1}^{7}(1 - \cos(\mathbf{v}_i, \mathbf{v}_j))$$
...
This calibration was derived by fitting to 5,000 random 7-word samples drawn from the navec vocabulary such that the median random score (raw approximately 87) maps to 78, matching the English DAT baseline. A separate set of 10,000 random draws was used for ceiling analysis (Section 5.6). All scores reported in this paper are adjusted scores unless otherwise noted. We verified that the nonlinear adjustment preserves the sign, direction, and approximate magnitude of all key correlations (test-retest: r_adj = .231 vs. r_raw = .264; word frequency: r_adj = -.051 vs. r_raw = -.058; time-on-task: r_adj = .103 vs. r_raw = .121).
...
Across all 21,159 submissions: M = 87.9, SD = 7.9, Mdn = 89.4, range = 48.6-104.8. The distribution was left-skewed (skewness = -1.65), with 75% of scores falling between 84.4 and 93.1. The median time spent was 86 seconds (M = 99s after excluding outliers below 10s and above 1,800s). The corpus of submitted words comprised 148,113 word-tokens and 16,952 unique lemmas.
...
## 3. Study 1: Cross-Linguistic Validation and Psychometric Properties

### 3.1 Rationale

Adapting the DAT to a new language raises two questions: (a) are scores comparable across languages, and (b) does the adapted version meet psychometric standards for reliability? We address both through cross-linguistic calibration and internal consistency analysis. Table 1 provides a side-by-side comparison of the key parameters of DAT-RU and the original English DAT.

**Table 1.** Comparison of DAT-RU and the original English DAT (Olson et al., 2021).
...
| Score formula | Mean cosine dist. x 100 | Power-law calibrated to English scale |
| Mean score (SD) | 78 (10) | 87.9 (7.9) |
| Score range | ~50-107 | 48.6-104.8 |
| Cronbach's alpha | Not reported | .899 |
| Split-half (Spearman-Brown) | Not reported | .696 |
...
### 3.2 Cross-Linguistic Calibration (A1)
...
However, reliability was strongly score-dependent due to a ceiling effect (Figure 1B). The bottom quintile (most diverse word sets) showed positive within-group reliability (r = .582, alpha = .933), while the top four quintiles -- compressed into a narrow score range -- showed near-zero or negative within-group reliability. This ceiling effect reflects the geometry of high-dimensional cosine distance: once words are drawn from sufficiently distinct categories, further increases in semantic distance become geometrically constrained.

### 3.4 Test-Retest Reliability (B1, A3)
...
## 4. Study 2: Practice Effects and the Sampling Model of Creativity
...
### 4.2 Practice Trajectory Analysis (B1, A3)

#### Method

We examined score trajectories across attempts using mixed-effects models. For the full sample (983 users with 2 or more attempts, 398 with 5 or more), we fit: score ~ attempt_number + (1 + attempt_number | user_agent) using restricted maximum likelihood. We also examined the distribution of within-user OLS slopes.
...
### 4.3 The i.i.d. Sampling Model (B2)

#### Method

We tested a null model: each attempt is an independent draw from the user's personal ability distribution, Normal(mu_user, sigma_user), with no learning between attempts. Under this model, the personal best after n attempts grows as E[max(X_1, ..., X_n)] -- a function of order statistics that increases concavely but requires no learning mechanism.

For 121 specific-UA users with 5 or more attempts (69 with 10 or more), we estimated (mu, sigma) from all attempts, simulated 1,000 i.i.d. replicates, and compared the observed personal-best curve with the i.i.d. prediction. Note that estimating (mu, sigma) from the same data used for testing introduces a conservative bias: if genuine learning were present, it would inflate sigma, producing an i.i.d. prediction with *more* variability than the true null -- making the test harder to reject. The fact that observed personal bests fall *below* the i.i.d. prediction (Section 4.3 Results) despite this bias strengthens the no-learning conclusion.

#### Results

The i.i.d. model fit the data well -- in fact, too well (Figure 2A). Observed personal bests grew at or below the rate predicted by random sampling:

- At attempt 5: observed PB was 0.59 points *below* the i.i.d. prediction (p = .004)
- At attempt 10: -1.52 below (p < .0001)
- At attempt 20: -3.09 below (p < .0001)
...
### 4.4 Optimal Stopping and the Cost of Persistence (E5)
...
Marginal gains followed the expected order-statistics decay (Figure 2B). Attempt 2 added approximately 3.5 points to the personal best; attempt 5 added 0.6; attempt 10 added 0.2. Time efficiency collapsed accordingly:
...
## 5. Study 3: Lexical Predictors of Creative Performance
...
Certain concrete, imageable words were massively overrepresented relative to their corpus frequency: "cat" (koshka) was used 343 times more often than predicted by corpus frequency, followed by "dog" (sobaka), "table" (stol), and "chair" (stul). These "DAT attractor" words are prototypically concrete, easily visualized nouns. Conversely, high-frequency abstract words ("time," "year," "matter") were virtually never used -- "semantic blind spots" reflecting a bias toward imageable over functional semantics.

### 5.3 Semantic Category Diversity (C2)
...
Category diversity was the strongest single predictor of DAT score (r = .473, p < .001; Figure 3A). Mean scores rose sharply with the number of unique categories: 1 category = 58.0, 3 categories = 80.7, 5 categories = 84.9, 6 categories = 86.1, with diminishing returns beyond 5 categories.
...
### 5.4 Positional and Anchor Effects (C3)
...
### 5.5 Within-Set Structural Topology (C4)
...
The most common "bottleneck pairs" -- close semantic associates appearing together -- were table-chair (31 occurrences), cat-dog (30), male_dog-female_dog (23), sea-sun (21), and tomcat-cat (17). A single such pair depressed the overall set score.
...
### 6.4 Lexical Accessibility and the Semantic Space

Study 3 paints a vivid picture of how most people navigate semantic space. The UMAP visualization (Figure 4) shows crowded zones around "cat," "dog," "table," and "sun" -- concrete, imageable nouns that come to mind easily and cluster together in embedding space. High scorers escape these zones by accessing rarer, more abstract, or more domain-specific terms, achieving uniform coverage across semantic space rather than clustering in familiar neighborhoods.

The "weakest link" finding has practical implications: a single pair of close associates (cat-dog, table-chair) is sufficient to drag a score from excellent to mediocre. This means that divergent thinking, as measured by the DAT, is as much about *avoiding* automatic associations as it is about *generating* distant ones -- a distinction with echoes in inhibitory accounts of creative cognition (Radel et al., 2015).

### 6.5 Cross-Linguistic Implications
...
Several limitations constrain interpretation. First, **no external validity measures** (AUT, RAT, Creative Achievement Questionnaire) were collected, preventing assessment of convergent validity. This is especially important given that at least one study has found weak convergent validity for the DAT in English (r = .03 with AUT; Forthmann & Bürkner, 2023). The semantic category analysis (r = .47 between category diversity and score) provides indirect construct validation but does not replace criterion-referenced validation. Second, **no demographic data** (age, gender, education, native language) were collected, precluding analyses of individual differences. Third, **user-agent-based identification** is imperfect. While sensitivity analyses showed key findings to be robust across identification confidence tiers, practice-effect analyses are necessarily approximate. Fourth, the sample was drawn from Russian-speaking social media users, likely skewing toward educated, tech-savvy adults, **limiting generalizability.** Fifth, the **ceiling effect** (75% of scores in a narrow range) restricts the test's ability to discriminate among high performers and contributes to score-dependent reliability.
...
Bai, H., Murayama, K., & Cromley, J. G. (2024). Using the divergent association task to measure divergent thinking in Chinese elementary school students. *Thinking Skills and Creativity, 52*, 101487.
...
Forthmann, B., & Bürkner, P.-C. (2023). The Divergent Association Test lacks convergent validity. *Creativity Research Journal*. Advance online publication.
...
Haase, J., Hanel, P. H. P., & Pokutta, S. (2025). S-DAT: A multilingual, GenAI-driven framework for automated divergent thinking assessment. In *Proceedings of the AAAI/ACM Conference on AI, Ethics, and Society*. arXiv:2505.09068.
...
Korobov, M. (2015). Morphological analyzer and generator for Russian and Ukrainian languages. *Communications in Computer and Information Science, 542*, 320-332.
...
Speer, R., Chin, J., & Havasi, C. (2018). ConceptNet 5.5: An open multilingual graph of general knowledge. In *Proceedings of the AAAI Conference on Artificial Intelligence, 31*(1).
...
### S3. Example Word Sets

**Table S1.** Example submissions illustrating high, typical, and low DAT scores, with English translations.

**Top-scoring sets (score > 104):**

| Score | Words (Russian) | Translation | Time |
|-------|----------------|-------------|------|
| 104.8 | вомбат, сталь, исследование, драма, шепот, пиджак, святки | wombat, steel, research, drama, whisper, jacket, Yuletide | 51s |
| 104.3 | прелесть, нейромедиатор, капот, империализм, усыновление, челядь, канонада | charm, neurotransmitter, hood, imperialism, adoption, servants, cannonade | 81s |
| 104.2 | человеконенавистничество, момент, триптофан, жизнь, фазенда, юань, пластика | misanthropy, moment, tryptophan, life, hacienda, yuan, surgery | 75s |

**Typical sets (score ~ 89):**

| Score | Words (Russian) | Translation | Time |
|-------|----------------|-------------|------|
| 89.1 | стюардесса, хлороформ, притяжение, алгоритм, скалолаз, зануда, муравьед | stewardess, chloroform, gravity, algorithm, climber, bore, anteater | 119s |
| 89.1 | рюкзак, хризантема, копыто, клей, лягушка, солнце, банка | backpack, chrysanthemum, hoof, glue, frog, sun, jar | 57s |
| 89.1 | кот, работа, гель, пальма, воск, струна, молекула | cat, work, gel, palm, wax, string, molecule | 153s |

**Bottom-scoring sets (score < 50):**

| Score | Words (Russian) | Translation | Time |
|-------|----------------|-------------|------|
| 48.6 | январь, февраль, март, апрель, май, июнь, июль | January, February, March, April, May, June, July | 27s |
| 48.6 | январь, декабрь, февраль, март, апрель, сентябрь, октябрь | January, December, February, March, April, September, October | 116s |

*Note.* The bottom-scoring sets illustrate the "weakest link" principle: listing words from a single semantic category (months) produces uniformly small pairwise distances. The 116-second example suggests a deliberate (not rushed) attempt that still failed due to category restriction.

### S4. User-Agent Sensitivity Analysis

User-agent strings were classified into three confidence tiers based on device model specificity: Specific (41% of records; identifiable device model, low collision risk), Moderate (26%; partial device information), and Generic (33%; desktop browsers or privacy-masked Android devices, higher collision risk). Key findings were stable across tiers: test-retest r = .225 (specific), .253 (moderate), .230 (generic). Among users with 50+ attempts, 65% of generic-UA "heavy users" showed suspicious temporal patterns (24-hour activity spread) consistent with multiple real individuals sharing a user-agent string. Full details in analysis_A3.
